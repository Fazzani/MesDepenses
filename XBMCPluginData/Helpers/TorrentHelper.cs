...
namespace XBMCPluginData.Helpers
{
    public class TorrentHelper
    {
        ...
        /// <returns></returns>
        public static TorrentInfo GetTorrentInfoAsync(string uri)
        {
            ...
            if (res != null)
            {
                ...
                if (infos != null)
                {
                    ...
                    {
                        TorrentFileName = infos.FirstOrDefault(x => x.Key == "name").Value.ToString()
                    };
                    if (infos.ContainsKey("length"))
                        torrentInfo.Length = infos.FirstOrDefault(x => x.Key == "length").Value.ToString();

                    if (infos.ContainsKey("files"))
                        torrentInfo.Files = (infos.FirstOrDefault().Value as BList).Select(x => x).Cast<BDict>().Select(x => new TorrentInfo{ TorrentFileName = (x.FirstOrDefault(k => k.Key == "path").Value as BList)[0].ToString(),Length = x.FirstOrDefault(k => k.Key == "length").Value.ToString()});
                    return torrentInfo;
                }
            }
            return default(TorrentInfo);
        }
    }
    ...
    public class TorrentInfo
    {
        ...
        public IEnumerable<TorrentInfo> Files { get; set; }
    }
}